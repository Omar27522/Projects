...
from src.ui.main_win.file_viewer_window import FileViewerWindow
from src.ui.main_window import MainWindow
from src.config import ConfigManager
from src.ui.window_icon_manager import WindowIconManager
...
def bring_to_front():
    ...
    try:
        ...
        hwnd = win32gui.FindWindow(None, "View Files")
        if hwnd:
...
if __name__ == "__main__":
    try:
        ...
        window_icon_manager = WindowIconManager()

        # Create and show the file viewer window as the main window
        root = FileViewerWindow(config_manager, window_icon_manager)
        
        # Create the label maker window but keep it hidden
        label_maker = MainWindow(root)
        label_maker.withdraw()
        
        # Add method to show label maker
        def show_label_maker():
            label_maker.deiconify()
            label_maker.lift()
            label_maker.focus_force()
        
        # Set the show_label_maker method
        root.show_label_maker = show_label_maker
