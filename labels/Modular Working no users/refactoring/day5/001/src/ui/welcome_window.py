...
class WelcomeWindow(tk.Tk):
    ...
    def open_sheets_dialog(self):
        ...
        # Check if dialog is already open
        if self.open_dialogs['google_sheets'] is not None:
            # If dialog exists but was destroyed, remove the reference
            if not self.open_dialogs['google_sheets'].winfo_exists():
                self.open_dialogs['google_sheets'] = None
            else:
                # Dialog exists, bring it to front
                dialog = self.open_dialogs['google_sheets']
        ...
        dialog = create_google_sheets_dialog_handler(
            self, 
            self.config_manager, 
            self._update_sheets_status_display
        )
        ...
        # Store reference to the dialog
        self.open_dialogs['google_sheets'] = dialog
        ...
        def on_dialog_close():
            ...
            # Reload config manager after dialog closes
            self.config_manager = ConfigManager()
            self._update_sheets_status_display()
