    ...
    def create_label_action(self):
        ...
        def print_label():
            ...
            try:
                ...
                # Log to the central shipping_records.txt file only
                timestamp = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                central_log_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), 'logs')
                central_log_file = os.path.join(central_log_dir, 'shipping_records.txt')
                
                # Create logs directory if it doesn't exist
                if not os.path.exists(central_log_dir):
                    os.makedirs(central_log_dir)
                
                # Write to central log file in the format expected by the Returns Data window
                with open(central_log_file, 'a') as f:
                    f.write(f"{timestamp} | Tracking: {tracking_number} | SKU: {sku} | Label: {barcode_filename}\n")
