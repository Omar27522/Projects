...
def create_sheets_status_display(parent, status_text="Not Connected", status_color="red", sheet_name=None, on_click=None):
    """
    ...
    # Make the label clickable if a click handler is provided
    if on_click and status_text == "Configured (Not Tested)":
        status_label.bind("<Button-1>", lambda e: on_click())
        status_label.bind("<Enter>", lambda e: status_label.config(cursor="hand2", font=("Arial", 8, "underline")))
        status_label.bind("<Leave>", lambda e: status_label.config(cursor="", font=("Arial", 8)))
