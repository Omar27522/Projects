...
class WelcomeWindow(tk.Tk):
    ...
    def _create_ui(self):
        ...
        # Verify the Google Sheets connection status on startup
        self._verify_sheets_connection_on_startup()
    ...
    def _create_version_label(self):
        ...
        version_label = create_version_label(self, "Ver. 1.0.1.2")
    ...
    def open_sheets_dialog(self):
        ...
        # Pass the _update_sheets_status_display method as the update callback
        sheets_dialog = create_google_sheets_dialog_handler(
            self, 
            self.config_manager,
            self._update_sheets_status_display
        )
        ...
        # Update the Google Sheets status display is now handled by the callback
    
    def _verify_sheets_connection_on_startup(self):
        """Verify the Google Sheets connection status on startup without showing any dialogs"""
        try:
            # Get Google Sheets configuration
            sheets_config = self.config_manager.settings
            
            # Only verify if the status is currently set to "Connected"
            if (hasattr(sheets_config, 'google_sheets_connection_status') and 
                ...
                if (not hasattr(sheets_config, 'google_sheet_url') or 
                    not sheets_config.google_sheet_url or 
                    not hasattr(sheets_config, 'google_sheet_name') or 
                    not sheets_config.google_sheet_name):
                    
                    # Configuration is missing, update status
                    sheets_config.google_sheets_connection_status = "Not Connected"
                ...
                is_valid, sheet_id = validate_sheet_url(sheets_config.google_sheet_url)
                
                if not is_valid:
                    # Invalid URL, update status
                    sheets_config.google_sheets_connection_status = "Connection Failed"
                    self.config_manager.save_settings()
                    self._update_sheets_status_display()
                    return
                
                # Test connection silently
                success, _ = test_sheet_connection(sheet_id, sheets_config.google_sheet_name)
            ...
            # If any error occurs, update status
            if hasattr(sheets_config, 'google_sheets_connection_status'):
                sheets_config.google_sheets_connection_status = "Connection Failed"
                self.config_manager.save_settings()
                self._update_sheets_status_display()
    
    def test_sheets_connection(self):
        """Test the Google Sheets connection directly from the welcome window"""
        try:
            # Get Google Sheets configuration
            sheets_config = self.config_manager.settings
            ...
            if (not hasattr(sheets_config, 'google_sheet_url') or 
                ...
                messagebox.showerror("Error", "Google Sheets is not properly configured.\n\nPlease configure it in Settings > Google Sheets Configuration.")
                return
            ...
            if not is_valid:
                messagebox.showerror("Error", f"Invalid Google Sheet URL: {sheet_id}")
                return
            
            # Test connection
            success, message = test_sheet_connection(sheet_id, sheets_config.google_sheet_name)
    ...
    def _update_sheets_status_display(self):
        ...
        if hasattr(self, 'sheets_status_label'):
            ...
            self.sheets_status_label.unbind('<Button-1>')
            self.sheets_status_label.unbind('<Enter>')
            self.sheets_status_label.unbind('<Leave>')
