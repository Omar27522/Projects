...
class MainWindow(tk.Tk):
    ...
    def view_directory_files(self):
        ...
        if hasattr(self, 'file_window') and self.file_window and self.file_window.winfo_exists():
            self.file_window.deiconify()  # Restore if minimized
        ...
        # Set initial topmost state based on saved setting
        self.file_window.attributes('-topmost', self.config_manager.settings.files_window_on_top)
        ...
        window_always_on_top = tk.BooleanVar(value=self.config_manager.settings.files_window_on_top)
        ...
        def toggle_window_on_top():
            ...
            window_top_btn.config(
                bg='#90EE90' if current_state else '#C71585',  # Green when active, Velvet when inactive
                relief='sunken' if current_state else 'raised'
            )
            # Save the setting
            self.config_manager.settings.files_window_on_top = current_state
            self.config_manager.save_settings()
            
            # If unpinning, ensure it's really unpinned
            if not current_state:
                # Double-check topmost is off after a short delay
                self.file_window.after(100, lambda: self.file_window.attributes('-topmost', False))
        ...
        self.Tooltip(window_top_btn, "Keep window on top")

        # Set initial state based on saved setting
        if window_always_on_top.get():
            toggle_window_on_top()
        else:
            # Ensure window starts unpinned if that's the saved state
            self.file_window.attributes('-topmost', False)
        ...
        # Add tooltip for magnifier button
        self.Tooltip(magnifier_btn, "Toggle text size")
        ...
        # Add tooltip for zoom button
        self.Tooltip(zoom_btn, "Cycle through preview sizes")

        # Add mirror print toggle button
        self.is_mirror_print = tk.BooleanVar(value=self.config_manager.settings.mirror_print)
        mirror_btn = tk.Button(top_frame, text=" 🖨️ ", bg='#32CD32' if self.config_manager.settings.mirror_print else '#C71585', relief='raised', width=3,
                             font=('TkDefaultFont', 14), anchor='center')

        def toggle_mirror_print():
            current_state = self.is_mirror_print.get()
            mirror_btn.config(
                bg='#32CD32' if current_state else '#C71585'
            )
        ...
        # Add tooltip for mirror print button
        self.Tooltip(mirror_btn, "Toggle mirror print")

        # Add Auto-minimize and auto-switch toggle button
        auto_minimize = tk.BooleanVar(value=self.config_manager.settings.auto_minimize)
        auto_minimize_btn = tk.Button(top_frame, text="⚡", width=3,
                                    bg='#C71585', relief='raised',
                                    font=('TkDefaultFont', 14))
        
        def toggle_auto_features():
            current_state = auto_minimize.get()
            if current_state:
                auto_minimize_btn.config(
            ...
            self.config_manager.settings.auto_minimize = current_state
            self.config_manager.save_settings()

        auto_minimize_btn.config(
            command=lambda: [auto_minimize.set(not auto_minimize.get()), 
                           toggle_auto_features()]
        )
        auto_minimize_btn.pack(side=tk.LEFT, padx=2)
        # Add tooltip for auto features button
        self.Tooltip(auto_minimize_btn, "Auto-minimize after opening/printing files\nAuto-switch to view files after label creation")

        # Set initial state
        if auto_minimize.get():
            toggle_auto_features()
                    ...
                    if is_upc_search:
                        ...
                        if self.is_auto_switch.get():
                            ...
                            self.file_window.after(375, lambda: [
                                ...
                                setattr(self, 'file_window', None),
                                self.input_vars['upc_code'].set(search_text),
                                self.deiconify(),  # Ensure main window is not minimized
        ...
        def update_last_print_time():
            ...
            if self.last_print_time:
                ...
                last_print_label.bind('<Button-1>', lambda e: show_last_printed_label())
            else:
        ...
        def show_last_printed_label():
            """Show the last printed label from the reference directory"""
            if self.last_printed_upc:
                # Look for the label in the LastPrintedLabel/assets/icons directory
                icons_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 
                                       'LastPrintedLabel', 'assets', 'icons')
                # Find any file containing the UPC
                for filename in os.listdir(icons_dir):
                    if self.last_printed_upc in filename and filename.endswith('.png'):
                        ...
                        try:
                            ...
                            if preview_width > 1 and preview_height > 1:
                                # Use current preview size setting
                                size_map = {1: 0.70, 2: 0.80, 3: 0.95}
                                size_multiplier = size_map[preview_size.get()]
                                preview_width = int(preview_width * size_multiplier)
                                preview_height = int(preview_height * size_multiplier)
                            ...
                            print(f"Failed to load preview: {str(e)}")
                            break
    ...
    def _create_tooltip_class(self):
        ...
        class ToolTip:
            ...
            def schedule(self, event=None):
                self.scheduled = self.widget.after(300, self.show)
    ...
    def _on_window_focus(self, focused_window):
        ...
        for window in self.app_windows:
            if window.winfo_exists():  # Check if window still exists
                ...
                else:  # Child windows (like view files window)
                    window.attributes('-topmost', self.config_manager.settings.files_window_on_top)
        
        # Just lift the focused window without making it topmost
        focused_window.lift()
    ...
    def _create_input_fields(self):
        ...
        def on_input_focus(event):
            ...
            self.deiconify()
            self.lift()
            event.widget.focus_set()  # Ensure the widget gets focus
        ...
        for idx, (label_text, key) in enumerate(labels):
            ...
            entry.bind("<Tab>", on_tab)  # Custom tab behavior
    ...
    def print_selected_file(self):
        ...
        try:
            ...
            if self.config_manager.settings.auto_minimize:
                self.file_window.iconify()
        except Exception as e:
