...
class DialogEngine {
  ...
  _showDialog(dialog, trigger = null) {
    ...
    requestAnimationFrame(() => {
      ...
      // Reset carousel loop count when opening a carousel dialog
    ...
    requestAnimationFrame(() => {
      ...
      if (this.currentlyOpenDialog) {
        ...
        if (this.currentlyOpenDialog.hasAttribute('data-carousel')) {
          ...
          // Reset active item to the first one
          const items = this.currentlyOpenDialog.querySelectorAll('.carousel-item');
          items.forEach(item => item.classList.remove('active'));
          const firstItem = this.currentlyOpenDialog.querySelector('.carousel-item[data-image-index="0"]');
